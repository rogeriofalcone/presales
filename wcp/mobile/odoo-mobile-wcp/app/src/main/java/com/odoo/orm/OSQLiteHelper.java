...
public class OSQLiteHelper extends SQLiteOpenHelper {
	...
	public List<OModel> baseModels() {
		...
		models.add(new ResUsers(mContext));
		models.add(new IrAttachment(mContext));
		models.add(new MailFollowers(mContext));
		models.add(new BaseAccount(mContext));
		models.add(new ProductModel(mContext));
		models.add(new DeliveryCylinderModel(mContext));
		models.add(new StockRouteModel(mContext));
		models.add(new AnalyticDeliveryModel(mContext));
		models.add(new EmployeeModel(mContext));
		return models;
	}
	...
	private void registerModelsClassPath(HashMap<String, String> modelClassPath) {
		...
		for (String key : modelClassPath.keySet()) {
			...
			List<String> server_cols = getColumns(modelClassPath.get(key), true);
			pfManager.putStringSet(key + ".server", server_cols);
			List<String> local_cols = getColumns(modelClassPath.get(key), false);
