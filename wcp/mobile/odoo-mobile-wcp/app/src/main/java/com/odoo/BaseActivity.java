...
import com.odoo.scan.PagerFragment;
...
public abstract class BaseActivity extends ActionBarActivity implements
	...
	private RelativeLayout mDrawerItemsListContainer;
    ...
    // BarcodeReader object for Intermec
	...
	protected enum SettingKeys {
		GLOBAL_SETTING, PROFILE, ACCOUNTS, ABOUT_US, SCAN
	}
	...
	private List<DrawerItem> setSettingMenu() {
		...
		String screensKey = "com.odoo.screens";
		String warehouse = "com.odoo.warehouse";

			Resources r = getResources();

		sys.add(new DrawerItem(warehouse, r.getString(R.string.title_scan_do), 0,
				R.drawable.delivery_icon, PagerFragment.newInstance(PagerFragment.TYPE.DO)));
		return sys;
	}
...
//		}
	...
	private void createNavDrawerItems(Bundle savedBundle) {
		mDrawerItemsListContainer = (RelativeLayout) findViewById(R.id.navdrawer_items_list);
		mLeftLinear = (LinearLayout) findViewById(R.id.left_linear);
		mRightLinear =(LinearLayout) findViewById(R.id.right_linear);
		...
		for (DrawerItem item : mNavDrawerItems) {
			...
			if((i%2)==0) {
				mLeftLinear.addView(mNavDrawerItemViews[i]);
			}else {
				mRightLinear.addView(mNavDrawerItemViews[i]);
			}
			++i;
		}
		mDrawerItemsListContainer.addView(mLeftLinear);
		mDrawerItemsListContainer.addView(mRightLinear);
	...
	protected void onNavDrawerItemClicked(final DrawerItem item, Bundle savedBundle) {
...
//				.findViewById(R.id.profile_cover_image);
	...
	private void populateAccountList(List<OUser> accounts) {
		...
		for (final OUser account : accounts) {
			...
			((ImageButton) itemView.findViewById(R.id.setting_button)).setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View view) {

//					getFragBundle(new Fragment(),
