...
public class OSyncHelper {
		...
		if (!mFinishedModels.contains(model.getModelName())
			...
			try {
				...
				JSONObject result = mOdoo.search_read(model.getModelName(),
						JSONUtils.getFields(model), domain.get(), 0, mSyncDataLimit,
						null, null);
				if (checkForCreateWriteDate
	 ...
	 * @return the o values
	 */
	public OValues createValueRow(OModel model, List<OColumn> columns,
	...
	public void clearAffectedIds(){
		mAffectedIds.clear();
	}
	
	public void addAffectedIds(List<Integer> list){
		mAffectedIds.addAll(list);
	}
	...
	public List<OValues> modelInfo(List<String> models) {
		...
		try {
			...
			JSONObject result = mOdoo.search_read("ir.module.module",
					JSONUtils.getFields(mModel), domain.get());
			JSONArray records = result.getJSONArray("records");
			for (int i = 0; i < records.length(); i++) {
				JSONObject record = records.getJSONObject(i);
	...
	public JSONObject callMethod(String modelName, String methodName, JSONObject args, Integer id) {
		try{
			return mOdoo.callMethod(modelName, methodName, args, id);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public JSONObject callMethod(String modelName, String methodName, JSONArray args){
		try{
			return mOdoo.call_kw(modelName, methodName, args);
		}catch (Exception e) {
			e.printStackTrace();
		}
		return null;
